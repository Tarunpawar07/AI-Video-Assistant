...
def render_step_bar(label: str, key: str, icon: str):
    ...
    timing = st.session_state.step_timings.get(key, "")
    timing_html = f'<span style="margin-left:auto;font-size:0.65rem;color:var(--success)">{timing}</span>' if timing else ""
    st.markdown(f"""
...
def get_download_content(result: dict) -> str:
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    return f"""AI MEETING ASSISTANT — REPORT
Generated: {now}
{'='*60}
...
if run_btn:
    if not source.strip():
        ...
        def timed_step(key, fn, *args, **kwargs):
            ...
            st.session_state.step_timings[key] = f"{elapsed:.1f}s"
        ...
        try:
            ...
            st.session_state.step_timings["extract"] = f"{time.time()-t0:.1f}s"
...
if st.session_state.result:
    ...
    # Title banner + download button on same row
    title_col, dl_col = st.columns([4, 1], gap="medium")
    with title_col:
        st.markdown(f"""
        <div class="card">
    ...
    with dl_col:
        ...
        report_text = get_download_content(r)
        st.download_button(
    ...
    with col1:
        ...
        <div class="card">
            ...
            <div class="card-content">{safe(r['summary'])}</div>
        </div>""", unsafe_allow_html=True)
    ...
    with c1:
        ...
        <div class="card">
            ...
            <div class="card-content">{safe(r['action_items'])}</div>
        </div>""", unsafe_allow_html=True)
    ...
    with c2:
        ...
        <div class="card">
            ...
            <div class="card-content">{safe(r['key_decisions'])}</div>
        </div>""", unsafe_allow_html=True)
    ...
    with c3:
        ...
        <div class="card">
            ...
            <div class="card-content">{safe(r['open_questions'])}</div>
        </div>""", unsafe_allow_html=True)
